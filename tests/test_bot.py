...
class TestHomework:
    ...
    def test_homework_const(self, tg_bot_logic_module):
        for const in self.HOMEWORK_CONSTANTS:
            check_utils.check_default_var_exists(tg_bot_logic_module, const)
        assert getattr(tg_bot_logic_module, "RETRY_PERIOD") == self.RETRY_PERIOD, (
            "Не изменяйте переменную `RETRY_PERIOD`, её значение должно "
            f"быть равно `{self.RETRY_PERIOD}`."
        )
        student_verdicts = getattr(tg_bot_logic_module, "WORK_VERDICTS")
        assert (
            student_verdicts == self.HOMEWORK_VERDICTS
        ), "Не изменяйте значение переменной `WORK_VERDICTS`."

    def test_bot_init_not_global(self, tg_bot_logic_module):
        for var in tg_bot_logic_module.__dict__:
            assert not isinstance(getattr(tg_bot_logic_module, var), telebot.TeleBot), (
                "Убедитесь, что бот инициализируется только в функции " "`main()`."
            )

    def test_logger(self, tg_bot_logic_module):
        assert hasattr(
            tg_bot_logic_module, "logging"
        ...
        hw_source = check_utils.get_clean_source_code(
            inspect.getsource(tg_bot_logic_module)
        )
    ...
    def test_request_call(self, monkeypatch, current_timestamp, tg_bot_logic_module):
        ...
        try:
            tg_bot_logic_module.get_api_answer(current_timestamp)
    ...
    def test_get_api_answers(
        self, monkeypatch, random_timestamp, current_timestamp, tg_bot_logic_module
    ):
        func_name = "get_api_answer"
        check_utils.check_function(
            tg_bot_logic_module,
        ...
        result = tg_bot_logic_module.get_api_answer(current_timestamp)
        assert isinstance(
            result, dict
    ...
    def test_get_not_200_status_response(
        self, monkeypatch, current_timestamp, response, tg_bot_logic_module
    ):
        func_name = "get_api_answer"
        check_utils.check_function(
            tg_bot_logic_module,
    ...
    def test_get_api_answer_with_request_exception(
        self, current_timestamp, monkeypatch, tg_bot_logic_module
    ):
        func_name = "get_api_answer"
        check_utils.check_function(
            tg_bot_logic_module,
    ...
    def test_parse_status_with_expected_statuses(self, tg_bot_logic_module):
        ...
        for status_key in self.HOMEWORK_VERDICTS.keys():
            ...
            result = tg_bot_logic_module.parse_status(test_data)
            assert isinstance(
                result, str
    ...
    def test_parse_status_with_unknown_status(self, tg_bot_logic_module):
        func_name = "parse_status"
        check_utils.check_function(
            tg_bot_logic_module,
            func_name,
        ...
        for hw in homework_with_invalid_status:
            try:
                tg_bot_logic_module.parse_status(hw)
    ...
    def test_parse_status_no_homework_name_key(self, tg_bot_logic_module):
        ...
        try:
            tg_bot_logic_module.parse_status(homework_with_invalid_name)
        except KeyError as e:
    ...
    def test_check_response(self, random_timestamp, tg_bot_logic_module):
        ...
        try:
            tg_bot_logic_module.check_response(valid_response)
    ...
    def test_check_invalid_response(self, response, tg_bot_logic_module):
        func_name = "check_response"
        check_utils.check_function(
            tg_bot_logic_module,
            func_name,
        ...
        if response.defected_key:
            try:
                tg_bot_logic_module.check_response(response.data)
            except KeyError as e:
    ...
    def test_send_message(
        self, monkeypatch, random_message, caplog, tg_bot_logic_module
    ):
        monkeypatch.setattr(tg_bot_logic_module, "PRACTICUM_TOKEN", "sometoken")
        monkeypatch.setattr(tg_bot_logic_module, "TELEGRAM_TOKEN", "1234:abcdefg")
        monkeypatch.setattr(tg_bot_logic_module, "TELEGRAM_CHAT_ID", "12345")
        ...
        with check_utils.check_logging(
            ...
            tg_bot_logic_module.send_message(bot, "Test_message_check")
            assert bot.chat_id, (
    ...
    def test_bot_initialized_in_main(self, tg_bot_logic_module):
        func_name = "main"
        check_utils.check_function(
            tg_bot_logic_module,
            func_name,
        ...
        main_source = check_utils.get_clean_source_code(
            inspect.getsource(tg_bot_logic_module.main)
        )
    ...
    def test_main_without_env_vars_raise_exception(
        ...
        monkeypatch.setattr(tg_bot_logic_module, "PRACTICUM_TOKEN", None)
        monkeypatch.setattr(tg_bot_logic_module, "TELEGRAM_TOKEN", None)
        monkeypatch.setattr(tg_bot_logic_module, "TELEGRAM_CHAT_ID", None)
        with check_utils.check_logging(
            caplog,
    ...
    def test_main_check_response_is_called(
        ...
        monkeypatch.setattr(tg_bot_logic_module, func_name, mock_check_response)
        with caplog.at_level(logging.WARN):
            try:
                tg_bot_logic_module.main()
            except SystemExit:
    ...
    def test_main_send_message_with_new_status(
        ...
        monkeypatch.setattr(tg_bot_logic_module, "send_message", mock_send_message)
        with caplog.at_level(logging.WARN):
            try:
                tg_bot_logic_module.main()
            except check_utils.BreakInfiniteLoop:
    ...
    def test_main_send_message_with_telegram_exception(
        ...
        if hasattr(tg_bot_logic_module, "TeleBot"):
            monkeypatch.setattr(tg_bot_logic_module, "TeleBot", MockedBotWithException)
    ...
    def test_docstrings(self, tg_bot_logic_module):
        for func in self.HOMEWORK_FUNC_WITH_PARAMS_QTY:
            check_utils.check_docstring(tg_bot_logic_module, func)
